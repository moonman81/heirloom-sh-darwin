# HOWTO — install and use Heirloom Bourne shell on Darwin
...
```sh
git clone https://github.com/moonman81/heirloom-sh-darwin
cd heirloom-sh-darwin
```
...
Gunnar Ritter maintained. Nothing in `heirloom-sh-darwin` alters
...
writes `.install-manifest-sh.txt` at the repo root. That
...
Reads `.install-manifest-sh.txt`, removes every listed file
